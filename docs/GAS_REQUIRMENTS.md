...
- clasp とローカルでのテストフレームワーク（jest + gas-mock など）を用意する。`GAS/` フォルダにコードを置き、`dist/` にビルドして `clasp push` で反映するワークフローを推奨。
- 単体テスト: ビジネスロジック（calcDaysUntil 等）は Node 環境でのユニットテストを用意すると品質が上がる。
